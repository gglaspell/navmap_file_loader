...
void NavmapFileLoader::on_initialize()
{
  ...
  node->declare_parameter<std::string>(plugin_name + ".frame_id", "map");
  node->get_parameter(plugin_name + ".frame_id", frame_id_);

  // Topic follows NavMapMapsManager convention: <node_fqn>/<plugin>/map
  // QoS matches NavMapMapsManager: plain depth=1 (NOT transient_local)
  navmap_pub_ = node->create_publisher<navmap_ros_interfaces::msg::NavMap>(
    plugin_name + "/map",
    rclcpp::QoS(1));
  ...
  if (!loaded_) {
    ...
    if (!navmap_ros::io::load_from_file(map_file_, navmap, &ec)) {
      RCLCPP_ERROR(
        node->get_logger(),
        "[NavmapFileLoader] Failed to load NavMap from '%s': %s",
        map_file_.c_str(), ec.message().c_str());
      return;
    }
    ...
    RCLCPP_INFO(
      node->get_logger(),
      "[NavmapFileLoader] NavMap loaded from '%s', waiting for subscriber...",
      map_file_.c_str());
  }

  // --- Step 2: re-publish each cycle until a subscriber connects ---
  // Cannot use transient_local: would cause silent QoS mismatch with downstream.
  if (published_) {
    return;
  }

  if (navmap_pub_->get_subscription_count() == 0) {
    return;
  }
